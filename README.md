...
Starter template for a paper's **data-analysis / artifact-generation pipeline**: the
kind of small Python project that turns a hand-curated corpus of papers (or any
other structured dataset backing a literature review / empirical study) into the
**figures**, **LaTeX tables**, and **BibTeX** a paper's LaTeX source consumes.
...
| File | Content |
|---|---|
| `corpus.csv` | 22 papers — title, year, venue, failure category, BibTeX key/entry |
| `taxonomy.csv` | The failure-type taxonomy: `FOCUS -> CATEGORY -> SUBCATEGORY -> FAILURE_TYPE` |
| `failure_observations.csv` | Which paper observed which failure type, at what severity — the join table the crosstab figure aggregates over |
...
article/
├── input/
│   ├── corpus.csv                # sample corpus — replace with your real papers
│   ├── taxonomy.csv               # sample taxonomy — replace with your real one
│   └── failure_observations.csv   # sample paper<->taxonomy join table
...
├── Fig1_sankey.py                   # Sankey diagram (Plotly): Category -> Venue flow
├── Fig2_stackedbar.py               # Stacked bar: papers per year, by category
├── Fig3_bubble.py                   # Bubble chart: Category x Year, bubble size = count
├── Fig4_distribution.py             # Bar + donuts: papers per year, venue-type/venue distribution
...
paper repo (and, optionally, a companion replication-package repo), point
...
| `ARTICLE_OUTPUT_BASE_DIR` | `OUTPUT_BASE_DIR` | Paper artifacts: figures (`figures/`), LaTeX tables (`sheets/`), and `biblio.bib` |
...
2. Update `palette.CATEGORY_ORDER`/`CATEGORY_COLORS`, `FOCUS_ORDER`/`FOCUS_COLORS`,
   and `SEVERITY_ORDER`/`SEVERITY_COLORS` (or add new palette axes) to match
   your paper's actual taxonomy.
3. Replace the placeholder chart(s) in `Fig1`–`Fig6` and the placeholder table
   in `generate_tables.py` with your paper's real figures and tables — keep
   loading data via `data_loader` and colors via `palette`.
4. Point `ARTICLE_OUTPUT_BASE_DIR` (via `.env`) at your actual paper repo once
...
5. Rename the `article` package (and update `pyproject.toml`'s
   `packages = [{ include = "article" }]` and `run_pipeline.py`'s
   `SCRIPTS_DIR`) to something specific to your paper, if you want.
