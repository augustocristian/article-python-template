...
- [.github/workflows/ci.yml](.github/workflows/ci.yml) — lints with flake8
  ...
  library meant to be packaged or published), and
  [sonar-project.properties](sonar-project.properties) excludes everything
  from the coverage metric (`sonar.coverage.exclusions=**/*`) so the default
  Quality Gate's coverage condition doesn't fail on an inherent 0%. If you add
  tests to a project generated from this template: remove that exclusion
  line, add `sonar.python.coverage.reportPaths=coverage-unit/coverage.xml`,
  and reinstate a pytest step in `ci.yml` that produces it.
