...
def draw_donut(ax, data: pd.Series, title: str, colors: list) -> None:
    if data.empty:
        ax.set_axis_off()
        ax.set_title(title, fontsize=ALL_TITLES, fontweight="bold")
        return
    ...
    for i, a in enumerate(autotexts):
        ...
        a.set_va("center")
        a.set_ha("center")
    ax.set_title(title, fontsize=ALL_TITLES, fontweight="bold", pad=DONUT_TITLE_PAD)
...
def main() -> None:
    ...
    # Panel A — Articles per Year, stacked by venue type
    ...
    ax_bar.set_title(TITLE_A, fontsize=ALL_TITLES, fontweight="bold", pad=12)
    ...
    ax_pie.set_title(TITLE_B, fontsize=ALL_TITLES, fontweight="bold", pad=PIE_TITLE_PAD)
