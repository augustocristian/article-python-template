...
from palette import (
    FOCUS_COLORS, TAXONOMY_CATEGORY_COLORS, CONNECTOR_COLOR,
    NEUTRAL_DARK, NEUTRAL_MUTED, NEUTRAL_FALLBACK, lighten,
)

# Forced text color per level (index = tree depth).
LEVEL_TEXT_COLOR = ["white", "white", "white", "white"]
# Text stroke (outline) per level — None = no stroke.
LEVEL_TEXT_STROKE = [None, None, "black", "black"]
# Lightening factor (0 = original color, 1 = white) applied per level, so every
# node under a Category inherits that Category's color as a progressively
# lighter tint (see palette.lighten()).
...
BOTTOM_MARGIN = 0.9
HEADER_Y = -0.55
LEAF_SPACING = 0.42               # vertical gap between adjacent leaf nodes
CAT_GAP = 0.14                    # extra gap between sibling categories
LEVEL_X = [1.0, 3.4, 6.2, 9.6]     # column x-centers, in inches
BOX_W = [1.7, 2.3, 2.6, 3.2]
BOX_H = [0.55, 0.34, 0.34, 0.30]

# ── Style (line widths / box styles) ──────────────────────────────────────────
...
class Node:
    ...
    @property
    def color(self) -> str:
        return lighten(self.base_color, LIGHTEN[self.level])

    @property
    def text_color(self) -> str:
        return LEVEL_TEXT_COLOR[self.level]

    def assign_y(self, counter: list) -> None:
...
def build_focus_node(focus_label: str, lookups: dict) -> Node:
    focus_node = Node(focus_label, 0, FOCUS_COLORS.get(focus_label, NEUTRAL_MUTED))
    counter = [0.0]

    ordered_cats = sorted(lookups["focus_cats"].get(focus_label, []))
    for cat_idx, category in enumerate(ordered_cats):
        cat_color = TAXONOMY_CATEGORY_COLORS.get(category, NEUTRAL_FALLBACK)
...
def draw_connector(ax, parent: Node, child: Node) -> None:
    ...
    ax.plot([px, mid_x, mid_x, cx], [parent.y, parent.y, child.y, child.y],
            color=CONNECTOR_COLOR, linewidth=CONNECTOR_LW,
...
def render_focus(focus_label: str, out_filename: str, lookups: dict) -> None:
    ...
    for x, label in zip(LEVEL_X, LEVEL_HEADERS):
        ax.text(x, HEADER_Y, label, ha="center", va="center",
                fontsize=FS_HEADER, fontweight="bold", color=NEUTRAL_DARK, zorder=5)
